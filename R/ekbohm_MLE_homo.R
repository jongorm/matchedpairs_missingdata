...
#' @title Ekbohm's MLE Test with Homoscedasticity
#' @description Performs an MLE based test for homoscedastic partially matched samples with a matrix or two-vector input that includes missing data.
#'
...
#' @param alt The test's alternative hypothesis. Defaults to 'two.sided'.
#'
#'
...
#'
#' ekbohm_MLE_homo(homo_50_0.7[,1], homo_50_0.7[,2], alt='less')
#'
#' #Example of function use, one matrix
#'
#' ekbohm_MLE_homo(homo_50_0.7, alt='less')
#'
#'
ekbohm_MLE_homo <- function(X, y=NULL, alt = c('two.sided', 'less', 'greater')) {
  ...
  sig <- (var(n1_dat[1,])*(n1-1)+var(n1_dat[2,])*(n1-1)+(1+r^2)*(var(n2_dat)*(n2-1)+var(n3_dat)*(n3-1)))/(2*(n1-1)+(1+r^2)*(n2+n3-2))
  V1 <- sig*((2*n1*(1-r)+(n2+n3)*(1-r^2))/((n1+n2)*(n1+n3)-n2*n3*r^2))

  Z_E <- (f*(mean(n1_dat[1,])-mean(n2_dat)) - g*(mean(n1_dat[2,])-mean(n3_dat)) + mean(n2_dat) - mean(n3_dat))/sqrt(V1)
  ...
  return(list(z.stat=Z_E, p.val=p.val))
}
